...
rule multiqc:
  ...
  input:
    ...
    kallisto = expand("kallisto.{cond}_{rep}",             cond=CONDITIONS, rep=REPLICATES, read=READ),
    salmon = expand("salmon.{cond}_{rep}",                 cond=CONDITIONS, rep=REPLICATES, read=READ)
  shell:
    "multiqc . -o {output}"
...
rule salmon_index:
    ...
    shell: "salmon index -t {input} -i {output} -k {params.kmer}"
